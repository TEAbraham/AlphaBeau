...
class MCTS:
	...
	def __init__(self, startpos=chess.Board(), iterations=None, iter_time=None,
		...
		self.__client = PredictClient(ADDRESS, PORT, 'Steve', int(self.version))
	...
	def search(self):
		...
		if not value_cache.get(leaf.position.fen()):
			try:
				value_cache[leaf.position.fen()] = self.__client.predict(util.build_off_heatmap(leaf.position))
			except:
				print("Prediction error, retrying...")
				value_cache[leaf.position.fen()] = self.__client.predict(util.build_off_heatmap(leaf.position))
		self.backprop(leaf, np.sum(util.build_off_heatmap(leaf.position)))
	...
	def expand_tree(self, leaf):
		if leaf.position:
			...
			for selected_move in moves:
				...
				prediction_cache[board.fen()] = pred_index
				new_edge = Edge(leaf, selected_move, 
					pred_index/np.sum(prediction_cache[board.fen()]))
